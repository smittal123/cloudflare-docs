...
:::note[Verify SSL certificates]
If your origins serve HTTPS traffic, ensure they have publicly trusted SSL certificates installed for the relevant hostnames. Cloudflare Origin CA certificates are only trusted by Cloudflare's edge and will cause certificate errors for direct client connections.
:::
